...
class CacheRepository:
    ...
    async def create_schema(self) -> None:
        ...
        logger.info("cache repository bootstrap start backend=%s", backend)
        ...
        logger.info("cache repository bootstrap complete backend=%s", backend)
    ...
    async def list_refresh_candidates(
        ...
        *,
        ...
        logger.info(
            "cache repository refresh candidates count=%d limit=%d lookback_hours=%.2f",
            len(rows),
            limit,
            lookback.total_seconds() / 3600.0,
        )
    ...
    async def aggregate_bucket_metrics(
        ...
        *,
        ...
        logger.info(
            "cache repository bucket aggregates buckets=%d lookback_hours=%.2f",
            len(aggregates),
            lookback.total_seconds() / 3600.0,
        )
        return aggregates
    ...
    async def fetch_prompt_stale_rates(
        ...
        *,
        ...
        logger.info(
            "cache repository stale rate fetch prompt_count=%d result_rows=%d",
            len(unique_hashes),
            len(rows),
        )
