...
class ThompsonSamplingConfig:
    """Configuration for a Thompson Sampling policy instance."""
...
@dataclass(frozen=True, slots=True)
class ThompsonSamplingSnapshot:
    """Serializable payload for Thompson policy state."""
...
class ThompsonSamplingPolicy:
    """Bayesian linear Thompson Sampling policy for contextual TTL selection."""
    ...
    def choose(
        ...
        """Sample per-action parameters and return the argmax action."""
        vector = self._vectorize(features)
        best_score = float("-inf")
        chosen = 0
        for action in range(self._config.num_actions):
            mean = np.linalg.solve(self._A[action], self._b[action])
            cov = self._config.sampling_variance * np.linalg.inv(self._A[action])
            sample = self._rng.multivariate_normal(mean, cov)
            score = float(sample @ vector)
            if score > best_score:
                best_score = score
                chosen = action
    ...
    def propensity(
        ...
        """Approximate action propensity via softmax over posterior means."""
        scores = self._score_all(features)
        logits = scores - np.max(scores)
        exp_logits = np.exp(logits)
        probs = exp_logits / np.sum(exp_logits)
    ...
    def snapshot(self) -> ThompsonSamplingSnapshot:
        """Generate a serializable view of the policy state."""
        matrices_payload = [matrix.tolist() for matrix in self._A]
        response_payload = [vector.tolist() for vector in self._b]
        return ThompsonSamplingSnapshot(
            config=self._config,
            feature_index=dict(self._indexer.index),
            matrices=matrices_payload,
            responses=response_payload,
    ...
    def load(cls, path: Path) -> "ThompsonSamplingPolicy":
        """Restore a Thompson Sampling policy from a JSON snapshot."""
        payload = json.loads(path.read_text(encoding="utf-8"))
        config = ThompsonSamplingConfig(**payload["config"])
        indexer = FeatureIndexer.from_mapping(
            payload["feature_index"],
            dimension=config.dimension,
            allow_growth=config.allow_feature_growth,
    ...
    def _score_all(self, features: FeatureVector | Mapping[str, float]) -> np.ndarray:
        """Compute posterior mean scores for all actions."""
        vector = self._vectorize(features)
        scores = np.zeros(self._config.num_actions, dtype=np.float64)
        for action in range(self._config.num_actions):
            theta = np.linalg.solve(self._A[action], self._b[action])
            scores[action] = float(theta.T @ vector)
        return scores
