...
class TTLPolicyManager:
    ...
    async def select_action(self, features: Mapping[str, float]) -> tuple[int, float]:
        ...
        async with self._lock:
            ...
            logger.info(
                "policy select action=%d propensity=%.4f %s",
                action,
                propensity,
    ...
    async def update(
        ...
        async with self._lock:
            ...
            logger.info(
                "policy update action=%d reward=%.4f updates_since_flush=%d %s",
                action,
                reward,
                self._updates_since_flush,
                self._summarize_features(features),
            )
            if (
                self._snapshot_path
    ...
    async def flush(self) -> None:
        ...
        async with self._lock:
            if self._snapshot_path is None:
                return
            self._persist_locked()
            logger.info("policy flush complete snapshot_path=%s", self._snapshot_path)
    ...
    def _persist_locked(self) -> None:
        ...
        self._updates_since_flush = 0
        logger.info("policy snapshot persisted path=%s", self._snapshot_path)
...
def build_policy_manager(config: PolicyRuntimeConfig) -> TTLPolicyManager:
    ...
    elif config.policy_type == "thompson":
        policy = _load_or_initialize_thompson(config, snapshot_path)
    else:
...
def _load_or_initialize_linucb(
...
def _load_or_initialize_thompson(
    config: PolicyRuntimeConfig,
    snapshot_path: Path | None,
) -> ThompsonSamplingPolicy:
    thompson_config = ThompsonSamplingConfig(
        num_actions=config.num_actions,
        dimension=config.feature_dimension,
        regularization=config.ts_regularization,
        sampling_variance=config.ts_sampling_variance,
        min_propensity=config.ts_min_propensity,
        allow_feature_growth=config.allow_feature_growth,
    )
    if snapshot_path and snapshot_path.exists():
        return ThompsonSamplingPolicy.load(snapshot_path)
    return ThompsonSamplingPolicy(config=thompson_config)
