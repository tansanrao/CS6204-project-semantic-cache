...
def _json_type(dialect: Dialect) -> TypeEngine:
    """Return a JSONB column when targeting Postgres, otherwise fall back to JSON."""
    if dialect.name == "postgresql":
        return JSONB  # type: ignore[return-value]
...
def cache_entry_table(dialect: Dialect | None = None) -> Table:
    ...
    if dialect is not None:
        ...
        json_type = JSONB  # type: ignore[assignment]
