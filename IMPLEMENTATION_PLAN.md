...
- [x] Prototype feature featurization module (`app/policy/features.py`) with deterministic unit tests. (22 Oct 2025 — covered by `tests/features/policy/test_features.py`)
...
- [x] Add batch job skeletons for pulling stale candidates and computing aggregate metrics. (22 Oct 2025 — `app/tasks/jobs.py`, repository helpers + `tests/ingest/ttl_refresh/test_jobs.py`)

### 2. Feature Engineering
- [x] Implement text preprocessing utilities (recency cues, structure metrics, normalization).
- [x] Integrate NER / entity type extraction (initially spaCy placeholder, pluggable backend). (22 Oct 2025 — default spaCy/regex recognizer wired via `FeatureExtractor`; see `app/policy/ner.py` & proxy wiring.)
...
- [x] Thompson Sampling variant scaffolding for future experiments. (22 Oct 2025 — `ThompsonSamplingPolicy` + manager wiring in `app/policy/` with unit coverage under `tests/features/policy/test_bandit.py`.)
...
- [x] Emit structured decision logs with bucket, features summary, and propensity. (22 Oct 2025 — Proxy now records TTL decisions via `SemanticCacheService.log_ttl_decision`)
...
- [ ] Start background refresh worker(s) from the Flask app serving process (thread/async loop) to process queued stale-hit evaluations.

### 5. Freshness & Reward Loop
- [x] Implement refresh worker to compare cached vs refreshed content, compute deltas, and determine staleness. (22 Oct 2025 — `app/tasks/worker.py`, covered by `tests/ingest/ttl_refresh/test_worker.py`)
...
- Proxy service integration tests (24 Oct 2025): `uv run pytest tests/features/test_proxy.py::test_proxy_caches_and_logs_decision`.
